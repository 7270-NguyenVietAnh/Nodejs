...
export default function BasicTabs({ user, setUser, getUser }) {
    ...
    const getProductInfo = async () => {
        try {
            ...
            setCategories(data.categories || []); // Assuming categories are part of the API response
        } catch (error) {
            console.log(error);
        }
    };
        ...
        <Box sx={{ width: '100%' }}>
            <Grid container spacing={2} direction={isSmallScreen ? 'column' : 'row'} padding={1}>
                <Grid item xs={12} sm={6} md={6} lg={3}>
                    <Widget numbers={categories.length} heading="Category" color="#9932CC" icon={<TbReportMoney />} />
                </Grid>
                <Grid item xs={12} sm={6} md={6} lg={3}>
            ...
            >
                <Tabs value={value} onChange={handleChange} aria-label="basic tabs example" style={{ overflowX: 'auto' }}>
                    ...
                    <Tab label={!isSmallScreen && 'Categories'} {...a11yProps(3)} iconPosition="start" icon={<AiOutlineShoppingCart fontSize={20} />} />
                </Tabs>
            </Box>
